...
export default function Dashboard() {
  ...
  const heatmapArray = sectorData?.sectors ? Object.entries(sectorData.sectors).map(([name, data]: any) => ({
    name,
    ...
    <div className="space-y-6 pb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard
          ...
          value={niftyPatterns?.results?.length || 0}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-[#111111] border border-[#2a2a2a] rounded-lg p-5">
            <h2 className="text-white font-bold tracking-wide uppercase text-sm mb-4">Market Technical Overview (^NSEI)</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Adv/Dec */}
            <div className="bg-[#111111] border border-[#2a2a2a] rounded-lg p-5">
              <h2 className="text-white font-bold tracking-wide uppercase text-sm mb-4">Market Breadth (Nifty 50)</h2>
              <div className="flex justify-between items-center px-4">
                 <div className="text-center">
                   <div className="text-3xl font-bold text-green-400 tabular-nums">{marketData?.market_breadth?.advances || 0}</div>
                   <div className="text-[#888] text-xs uppercase font-bold tracking-wider mt-1">Advances</div>
                 </div>
                 <div className="text-center">
                   <div className="text-3xl font-bold text-[#666] tabular-nums">{marketData?.market_breadth?.unchanged || 0}</div>
                   <div className="text-[#888] text-xs uppercase font-bold tracking-wider mt-1">Neutral</div>
                 </div>
                 <div className="text-center">
                   <div className="text-3xl font-bold text-red-400 tabular-nums">{marketData?.market_breadth?.declines || 0}</div>
                   <div className="text-[#888] text-xs uppercase font-bold tracking-wider mt-1">Declines</div>
                 </div>
              </div>
              
              <div className="w-full bg-[#1a1a1a] h-3 mt-6 rounded-full overflow-hidden flex">
                <div className="bg-green-500 h-full" style={{ width: `${(marketData?.market_breadth?.advances / 50) * 100 || 50}%` }}></div>
                <div className="bg-[#444] h-full" style={{ width: `${(marketData?.market_breadth?.unchanged / 50) * 100 || 0}%` }}></div>
                <div className="bg-red-500 h-full" style={{ width: `${(marketData?.market_breadth?.declines / 50) * 100 || 50}%` }}></div>
              </div>
            </div>

            {/* FII / DII */}
            <div className="bg-[#111111] border border-[#2a2a2a] rounded-lg p-5 flex flex-col justify-center">
              <h2 className="text-white font-bold tracking-wide uppercase text-sm mb-4">Institutional Flow (₹ Cr)</h2>
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-[#888] font-semibold">FII Net</span>
                  <span className="text-green-400 font-mono font-bold">+{marketData?.fii_dii?.fii_net || "1,132"}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-[#888] font-semibold">DII Net</span>
                  <span className="text-green-400 font-mono font-bold">+{marketData?.fii_dii?.dii_net || "1,109"}</span>
                </div>
              </div>
              <p className="text-[#666] text-xs mt-4 text-center">Data updated daily at 6:00 PM IST</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#111111] border border-[#2a2a2a] rounded-lg p-5 flex flex-col h-[350px]">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2">
              {!signalsData ? (
                 <div className="h-full flex items-center justify-center text-[#888]">Loading AI radar...</div>
              ) : signalsData.signals.length === 0 ? (
                 <div className="h-full flex items-center justify-center text-[#666]">No high-confidence signals today.</div>
              ) : (
          ...
          {/* Heatmap */}
          <div className="bg-[#111111] border border-[#2a2a2a] rounded-lg p-5">
            <h2 className="text-white font-bold tracking-wide uppercase text-sm mb-4">Sector Heatmap</h2>
