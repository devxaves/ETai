  ...
  const onDrop = async (acceptedFiles: File[]) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/portfolio/upload`,
        {
          method: "POST",
          body: formData,
        },
      ...
      const concentration = data?.analysis?.sector_concentration || {};
      const allocations = Object.entries(concentration).map(([name, pct]) => ({
        name,
        pct: Number(pct) || 0,
      }));

      setPortfolioData({
        healthScore: Number(data?.analysis?.health_score || 0),
        riskLevel: data?.analysis?.risk_level || "Unknown",
        overlapPct: Number(data?.analysis?.overlap_pct || 0),
        totalValue: Number(data?.portfolio?.total_value || 0),
        totalInvested: Number(data?.portfolio?.total_invested || 0),
        totalGainPct: Number(data?.portfolio?.total_gain_pct || 0),
        allocations,
      });
    } catch (err) {
    ...
    <div className="h-full flex flex-col pt-4 px-4 overflow-hidden gap-6">
      <div className="flex justify-between items-center mb-0">
        <div>
          ...
          <p className="text-[#888888] text-sm mt-1">
            ChromaDB RAG implementation providing personalized insights into
            your CAS/CAMS statements.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)]">
          ...
          {!sessionId ? (
            ...
            >
              ...
              {isUploading ? (
                ...
                <>
                  <svg
                    className="w-12 h-12 text-[#666] mb-4"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                  ...
                  <p className="text-lg text-white font-bold mb-2">
                    Upload CAS / CAMS Statement
                  </p>
                  <p className="text-sm text-[#888888] mb-6">
                    Drag & drop your PDF or CSV file here, or click to browse
                  </p>
                  ...
                  <p className="text-xs text-[#555] mt-6">
                    All parsing occurs securely. No data is stored long-term.
                  </p>
                </>
              )}
            </div>
            ...
            <>
              ...
              <div className="bg-[#111] border border-[#2a2a2a] rounded-lg p-6 flex flex-col gap-6">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-[#888] text-sm uppercase font-bold tracking-wider mb-1">
                      Portfolio Health
                    </h3>
                  ...
                  <div className="w-16 h-16 rounded-full border-4 border-green-500 flex items-center justify-center">
                    <span className="text-green-500 font-bold text-xs text-center">
                      {portfolioData?.riskLevel || "N/A"}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h4 className="text-[#666] text-xs font-bold uppercase mb-1">
                      Total Value
                    </h4>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-green-400 tabular-nums">
                      {portfolioData?.totalGainPct?.toFixed(2) || "0.00"}%
                      <span className="text-xs ml-2 bg-green-500/20 px-1 py-0.5 rounded">
                        Overlap {portfolioData?.overlapPct?.toFixed(1) || "0.0"}
                        %
                      </span>
                    </p>
                  </div>
              ...
              <div className="bg-[#111] border border-[#2a2a2a] rounded-lg p-6">
                <h3 className="text-[#888] text-sm uppercase font-bold tracking-wider mb-4">
                  Risk Allocation
                </h3>
        ...
        <div className="w-full lg:w-7/12 h-full pb-4 pr-1">
          <div
            className={`h-full opacity-100 transition-opacity duration-500 ${!sessionId ? "pointer-events-none filter blur-sm grayscale" : ""}`}
          >
            <ChatInterface
              sessionId={sessionId}
              onReset={() => {
