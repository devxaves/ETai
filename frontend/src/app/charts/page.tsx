    ...
    if (data.patterns) {
      patternMarkers = data.patterns.map((p: any) => {
        ...
        return {
          ...
          text: p.display_name || p.pattern_name,
        };
      });
    }
  }
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[#888888] text-sm mt-1">
            Over 60 candlestick patterns automatically detected and backtested
        ...
        <div className="w-1/3 flex flex-col gap-6 h-full p-2 overflow-y-auto">
          {data?.patterns && data.patterns.length > 0 ? (
            data.patterns.map((pattern: any, idx: number) => {
              const isBullish = Boolean(pattern.is_bullish);
              return (
                <div
                  key={idx}
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${isBullish ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-red-500/20 text-red-400 border border-red-500/30"}`}
                    >
                  ...
                  <p className="text-sm text-[#cccccc] leading-relaxed mb-4">
                    {pattern.explanation ||
                      "This pattern indicates a potential reversal in the current trend based on the latest 3 trading sessions."}
                  </p>
                </div>
              );
            })
            ...
            <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-6 text-center text-[#888888]">
              No significant candlestick patterns detected in the recent
              timeframe.
            </div>
          )}
