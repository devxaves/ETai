...
export default function SignalFeed() {
  ...
  if (error)
    return (
      <ErrorBoundary
        fallback={
          <div className="p-4 text-red-500 border border-red-500/20 bg-red-500/5 rounded">
  ...
  const filteredSignals = signals.filter((s) => {
    if (filter === "high") return s.confidence_score >= 70;
    if (filter === "medium") return s.confidence_score >= 40;
    return true; // all
  });
      ...
      <div className="w-full md:w-5/12 lg:w-1/3 flex flex-col h-full bg-[#111111] border border-[#2a2a2a] rounded-lg overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {filteredSignals.length === 0 ? (
            <div className="text-center text-[#888888] mt-10 text-sm">
      ...
      <div className="w-full md:w-7/12 lg:w-2/3 h-full border border-[#2a2a2a] bg-[#111111] rounded-lg overflow-hidden flex flex-col">
        {!selectedSignal ? (
          <div className="flex-1 flex flex-col items-center justify-center text-[#666666]">
            ...
            <p className="text-lg font-medium text-[#888888]">
              Select a signal to view details
            </p>
            <p className="text-sm mt-2 max-w-sm text-center">
              The AI Engine provides a comprehensive breakdown of why this
              signal fired and its historical precedent.
            </p>
          </div>
        ) : (
              ...
              <div className="bg-[#0a0a0a] rounded-lg p-6 border border-[#2a2a2a] shadow-inner mb-8">
                <div className="flex items-center justify-between mb-4 border-b border-[#2a2a2a] pb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono bg-[#1a1a1a] border border-[#2a2a2a] px-2 py-0.5 rounded text-blue-400">
                      Claude-3.5-Sonnet
                    </span>
                  </div>
                </div>
                ...
                <div className="prose prose-invert prose-green max-w-none prose-p:leading-relaxed prose-headings:font-semibold prose-a:text-green-400">
                  <ReactMarkdown>
                    {selectedSignal.explanation ||
                      "*No detailed explanation available for this signal yet.*"}
                  </ReactMarkdown>
                </div>
              </div>
            </div>
